    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[95vw] lg:max-w-[1200px] max-h-[90vh] flex flex-col p-0">
        ...
        {isCheckingApproval ? (
          ...
          // Approved - Dashboard layout with small left column for personal info, big right column for chats
          <div className="flex-1 flex min-h-0">
            {/* Left Sidebar - Personal Information (Small) */}
                    ...
                    <div className="bg-gradient-to-br from-green-50 to-emerald-50 border border-green-200 rounded-lg p-4 shadow-sm">
                      ...
                      <div className="text-xs text-gray-600 space-y-1">
                        <div className="flex justify-between">
                          ...
                          <span className="font-semibold">{counselorProfile.totalCases || 0}</span>
                        </div>
                        <div className="flex justify-between">
                          <span>Completed:</span>
                          <span className="font-semibold">{counselorProfile.completedCases || 0}</span>
                        </div>
                      </div>
                    </div>
                  )}
                    ...
                    <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-3">
                      <div className="flex items-start gap-2">
                        ...
                        <div className="flex-1">
                          <p className="font-semibold text-xs text-yellow-800">Pending Changes</p>
                          <div className="mt-1 space-y-0.5 text-xs text-yellow-700">
                            {counselorProfile.pendingChanges.bookingFee !== undefined && (
                              <p>• Fee: ${counselorProfile.pendingChanges.bookingFee.toFixed(2)}</p>
                            )}
                      ...
                      {isEditingProfile ? (
                        <div className="space-y-3">
                          <div className="space-y-1">
                            <Label className="text-xs">Booking Fee (USD)</Label>
                            <Input
                              type="number"
                          ...
                          <div className="space-y-1">
                            ...
                            <div className="grid grid-cols-1 gap-1.5 max-h-32 overflow-y-auto border rounded p-2 text-xs">
                              {categories.map((cat) => (
                                <label
                                  key={cat.id}
                        ...
                        <div className="space-y-2 text-xs">
                          ...
                          <div className="py-1.5">
                            <span className="text-gray-600 block mb-1.5">Specializations:</span>
                            <div className="flex flex-wrap gap-1">
                              {counselorProfile.specializations?.map((spec) => (
                                <Badge key={spec} variant="secondary" className="text-xs h-5">
                                  {spec}
                                </Badge>
                              ))}
                            </div>
              ...
              <ScrollArea className="flex-1">
                {!isOnline ? (
                  <div className="p-12 text-center">
                    <Circle className="h-16 w-16 text-gray-300 mx-auto mb-3" />
                    <p className="text-base text-gray-500 font-medium">Go online to see chats</p>
